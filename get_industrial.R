...
#   dsn <- paste0("05_data_output/industrial_landuse/", city_name, ".gpkg")
#   segment <- st_read(file_path)
#   bb <- segment %>%
#     st_transform(4326) %>%
#     st_bbox()
#   industrial <- opq(bbox = bb, timeout = 120) %>%
#     add_osm_feature(key = "landuse", value = c("industrial", "quarry")) %>%
#     osmdata_sf()
#   
#   
#   industrial_sf <- industrial$osm_polygons %>%
#     st_transform(crs) %>%
#     select(osm_id, name, landuse, geometry)
#   st_write(industrial_sf, dsn, layer = "industrial", delete_layer = TRUE)
#   
#   industrial_clean <- industrial_sf %>%
#     st_make_valid() %>%
#     st_geometry() %>%
#     st_union() %>%
#     st_collection_extract("POLYGON")
#   industrial_clean <- st_as_sf(industrial_clean)
#   names(industrial_clean)[names(industrial_clean) == "x"] <- "geometry"
#   st_geometry(industrial_clean) <- "geometry"
#   st_write(industrial_clean, dsn, layer = "cleaned", delete_layer = TRUE)
# }
